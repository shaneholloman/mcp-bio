"""CLI and raw-MCP parity for the rich author papers page (ticket 1143).

The fixture redirects the real Semantic Scholar client through
``BIOMCP_S2_BASE`` and records every request path.  The same command runs
through the direct CLI and through the raw ``biomcp`` MCP tool; outputs must
be byte-identical and the provider request log must contain exactly one
author-papers request per invocation with no graph, paper-detail, batch, or
Europe PMC traffic.
"""
...
RELEASE_BIN = Path(os.environ.get("BIOMCP_BIN", REPO_ROOT / "target" / "debug" / "biomcp"))
