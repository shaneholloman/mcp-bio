...
| Drug WHO regional context | WHO finished-pharmaceutical-products CSV + WHO active-pharmaceutical-ingredients CSV (local downloads) | `https://extranet.who.int/prequal/medicines/prequalified/finished-pharmaceutical-products/export?page&_format=csv`, `https://extranet.who.int/prequal/medicines/prequalified/active-pharmaceutical-ingredients/export?page&_format=csv` | No | Supports `search/get drug --region who|all`, WHO-filtered structured `search drug --region who`, and WHO-only `--product-type <finished_pharma|api>` filters; auto-downloads both files into `BIOMCP_WHO_DIR` or the platform data directory on first use and `biomcp who sync` force-refreshes the local exports |
| Drug vaccine identity bridge | CDC CVX code set + CDC trade-name map + CDC MVX manufacturer table (local downloads) | `https://www2.cdc.gov/vaccines/iis/iisstandards/downloads/cvx.txt`, `https://www2.cdc.gov/vaccines/iis/iisstandards/downloads/TRADENAME.txt`, `https://www2.cdc.gov/vaccines/iis/iisstandards/downloads/mvx.txt` | No | Supports omitted-`--region` plain-name vaccine search plus explicit `search drug <name> --region eu|all` when MyChem identity resolution misses; auto-downloads the bundle into `BIOMCP_CVX_DIR` or the platform data directory on first use, refreshes stale files after 30 days, and `biomcp cvx sync` force-refreshes the local CDC data; `--region us` stays outside this path |
| Diagnostic | NCBI Genetic Testing Registry bulk exports (local downloads) | `https://ftp.ncbi.nlm.nih.gov/pub/GTR/data/test_version.gz`, `https://ftp.ncbi.nlm.nih.gov/pub/GTR/data/test_condition_gene.txt` | No | Supports `search diagnostic` and `get diagnostic`; auto-downloads both required files into `BIOMCP_GTR_DIR` or the platform data directory on first diagnostic use, refreshes stale files after 7 days, and `biomcp gtr sync` force-refreshes the local GTR bundle |
...
2. Run `biomcp health` to inspect local readiness rows such as EMA local data, WHO Prequalification local data, CDC CVX/MVX local data, GTR local data, and cache dir
