...
fn main() -> Result<()> {
    ...
    let report = match cli.command {
        Command::Jats { input, output } => run_jats(&input, &output, started),
        Command::Html {
            input,
            base_url,
            output,
        } => run_html(&input, &base_url, &output, started),
        Command::Pdf {
            engine,
            input,
            output,
            page_limit,
        } => run_pdf(engine, &input, &output, page_limit, started),
    };
...
fn run_jats(input: &Path, output: &Path, started: Instant) -> Result<ProbeReport> {
    let xml = fs::read_to_string(input).with_context(|| format!("read {}", input.display()))?;
    let result = jats_to_markdown(&xml);
...
fn run_html(input: &Path, base_url: &str, output: &Path, started: Instant) -> Result<ProbeReport> {
...
fn jats_to_markdown(xml: &str) -> Result<(String, Value)> {
...
fn markdown_metrics(markdown: &str) -> Value {
...
fn score_jats(metrics: &Value) -> u8 {
    let mut score = 1;
...
fn score_html(metrics: &Value) -> u8 {
    let mut score = 1;
...
fn score_pdf(metrics: &Value) -> Value {
