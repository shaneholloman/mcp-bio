...
- `-k/--keyword` is provider-neutral: use `--gene`, `--disease`, `--drug`, `--author`, or `--journal` instead of field syntax. Reserved `gene:`, `disease:`, and `drug:` labels are rejected at the start or after whitespace/`(` unless a literal double-quote byte is immediately before the label. Put that byte before every reserved label intended as prose; a whole-value quote does not protect a later label. Ordinary biomedical brackets and colons remain literal keyword text. Shell/JSON quoting alone does not add runtime quote bytes.
