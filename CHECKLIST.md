...
- 2026-06-17: OpenTargets drug sections cleanup. Split `drug_sections` into a
  ...
  `cargo clippy --lib --tests -- -D warnings` → pass; `git diff --check` →
  pass.
- 2026-06-17: OpenTargets disease-associated-targets cleanup. Split disease
  search and associated-target lookup into pure GraphQL request plans plus pure
  response parsers, then replaced the disease-associated-target mock-server
  tests with construction/parsing tests over in-memory JSON. Target
  druggability, target clinical, and prevalence OpenTargets tests still use
  mock servers and remain for later slices. Checks:
  `cargo nextest run -E 'test(/sources::opentargets::tests::/)'` → 18/18
  pass; `cargo check` → pass;
  `cargo clippy --lib --tests -- -D warnings` → pass; `git diff --check` →
  pass.
- 2026-06-17: OpenTargets target-druggability cleanup. Split target search and
  ...
  `cargo clippy --lib --tests -- -D warnings` → pass; `git diff --check` →
  pass.
- 2026-06-17: OpenTargets target clinical cleanup. Split target clinical lookup
  into a pure GraphQL request plan plus a pure response parser, then replaced
  the target clinical mock-server tests with construction/parsing tests over
  in-memory JSON. Disease prevalence and two older OpenTargets fixture-style
  mock-server tests remain for later slices. Checks:
  `cargo nextest run -E 'test(/sources::opentargets::tests::/)'` → 22/22
  pass; `cargo check` → pass;
  `cargo clippy --lib --tests -- -D warnings` → pass; `git diff --check` →
  pass.
- 2026-06-17: OpenTargets disease prevalence cleanup. Split disease prevalence
  lookup into a pure GraphQL request plan plus a pure response parser, then
  replaced the prevalence mock-server test with construction/parsing tests over
  in-memory JSON. Removed the now-unused OpenTargets ad hoc `post_json` and
  GraphQL request struct so OpenTargets uses the shared `RequestPlan` send path.
  Two older OpenTargets fixture-style mock-server tests remain for later
  cleanup. Checks:
