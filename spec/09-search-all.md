...
```bash
bin="${BIOMCP_BIN:-biomcp}"
out="$("$bin" search all -g BRAF --debug-plan --limit 3 2>/dev/null)"
...
json_out="$("$bin" --json search all -g BRAF --debug-plan --limit 3 2>/dev/null)"
...
keyword_json="$("$bin" --json search all -k 'Hirschsprung disease' --debug-plan --limit 3 2>/dev/null)"
